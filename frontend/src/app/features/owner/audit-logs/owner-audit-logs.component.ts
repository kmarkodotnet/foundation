...
import { OwnerAuditLogService, OwnerAuditLogEntry } from '../services/owner-audit-log.service';
...
@Component({
          ...
          <mat-card-content>
            @if (entries().length === 0) {
              ...
              <table mat-table [dataSource]="entries()" class="full-width">
                ...
                <ng-container matColumnDef="foundationName">
                  <th mat-header-cell *matHeaderCellDef>Alapítvány</th>
                  <td mat-cell *matCellDef="let row">{{ row.foundationName ?? '—' }}</td>
                </ng-container>
  ...
  styles: [`
    ...
    .bg-icon { font-size: 16px; width: 16px; height: 16px; }
  `],
})
export class OwnerAuditLogsComponent implements OnInit {
  private readonly service = inject(OwnerAuditLogService);
  private readonly foundationService = inject(OwnerFoundationService);
  private readonly destroyRef = inject(DestroyRef);

  readonly loading = signal(false);
  readonly exporting = signal(false);
  readonly entries = signal<OwnerAuditLogEntry[]>([]);
  readonly result = signal<PagedResult<OwnerAuditLogEntry> | null>(null);
  readonly foundations = signal<FoundationListItem[]>([]);
  readonly columns = ['createdAt', 'userEmail', 'foundationName', 'entityType', 'action'];

  page = 1;
  pageSize = 50;

  readonly filterForm = new FormGroup({
    foundationId: new FormControl(''),
  ...
  ngOnInit(): void {
    this.foundationService.list()
      ...
      .subscribe((data) => this.foundations.set(data));
    this.load();
  }
  ...
  load(): void {
    ...
    this.service.list({
      ...
      foundationId: v.foundationId || undefined,
    ...
    this.service.exportCsv({
      ...
      .subscribe({
        next: (blob) => {
          ...
          a.href = url;
          a.download = `owner_audit_${date}.csv`;
          a.click();
